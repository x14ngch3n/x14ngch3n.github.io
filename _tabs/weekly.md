...
本周四开始连续三天有中国计算机大会 CNCC 2022，我选择了三个感兴趣的论坛参与。第一次参加这样的大会，可惜赶上了线上，希望明年还有机会参加。连着听了两天的会议，觉得收获最多的竟然是《CCF 优博的培养》。和专业知识相关的，除了《数据驱动的开源漏洞挖掘与修复》都带有太多的理论部分，没有基础的话很难听懂。所以我又去学习了一下包括 lambda 演算在内的知识，希望之后看回放能看懂。
...
中途还琢磨了一下 GNU TeXmacs 这样的科研协作神器，打算之后使用它来写作学术报告等材料，从工具的层面进一步接近一个学术人（差生文具多 hhh）。
...
三月份，有段时间继续在刷 pwn.college，现在把基础部分都做完了，可以放心地进入 pwn 知识的海洋了。同时还准备了开题报告，VoIS 演讲。三月底，我先后做了几件大事：准备 Goshawk-tutorial，准备 XCTF Finals，投递/面试实习。平常的时间，我还看完了 CS 3110 的前三章，对 OCaml 有了一个基本的了解，并且一直在看候捷的 C++ STL。
...
这样不行啊！！！到了 6 月，实习基本就要开始了。这两个月除了论文的工作我真的是不能再做别的了，CTF 也就休息的时候打打就好，基础知识的积累也就每天慢慢推进就好，不能再花太多时间在上面了。
...
没想到五一节期间发生了这么多事情，我几乎脱产，就把 pwn.college 的 web 部分做完了。我还联系了三个 OSPP 2023 的项目，如果实习没找到的话或许可以申请一个。

## 2023.05.08 - 2023.05.12

这周开组会被老吴推荐到了桂老师那里，之后要每周去那边汇报了。终于有个年轻老师来 push 我了，希望自己能够珍惜机会，不要再像之前一样自以为是/懒惰了。这周还把 pwn.college 逆向工程的部分做完，并开始正式进入 pwn 的部分了，感觉之后的进度会越来越快，也会真正学到关于 pwn 的知识的。
